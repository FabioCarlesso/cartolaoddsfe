...
- **Nome do `/etc/hosts` vira IP no boot, e só sob pedido** — o resolver do nginx fala DNS e só DNS: não consulta o `/etc/hosts`. Sem tratamento, o padrão de desenvolvimento deste repo quebraria, porque `host.docker.internal` existe **apenas** ali (posto pelo Docker Desktop ou pelo `extra_hosts` no Linux) e todo `/api/` responderia 502 `Host not found`. O entrypoint troca o nome pelo IP do `/etc/hosts` quando o nome vem de lá — correto para esse caso, porque a entrada é estática pela vida do container. Mas o comportamento é **opt-in e desligado por omissão**, ligado só pelo `docker-compose.yml` do desenvolvimento: fixar IP na subida é o defeito que este entrypoint existe para corrigir, e se a plataforma de produção puser o hostname do backend no `/etc/hosts` do container — o que não se controla daqui — um padrão "ligado" ressuscitaria o 504 silencioso no deploy seguinte. Produção não define a variável e não depende de premissa alguma sobre o `/etc/hosts` de lá
