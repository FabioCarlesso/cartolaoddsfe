...
- **Nome do `/etc/hosts` vira IP no boot** — o resolver do nginx fala DNS e só DNS: não consulta o `/etc/hosts`. Sem tratamento, o padrão de desenvolvimento deste repo quebraria, porque `host.docker.internal` existe **apenas** ali (posto pelo Docker Desktop ou pelo `extra_hosts` no Linux) e todo `/api/` responderia 502 `Host not found`. Então o entrypoint troca o nome pelo IP do `/etc/hosts` quando — e somente quando — o nome vem de lá. Isso não reabre o problema do cache: uma entrada de `/etc/hosts` é estática pela vida do container, não há o que reconsultar. O caminho que precisava de re-resolução é o do DNS da plataforma, e esse segue intocado
